...
- 统一显示：不做性能限制——无 reduced-motion 跳过、无后台暂停，效果对所有人一致（2026-08-10 依用户反馈移除）
- 零新依赖：vanilla JS + CSS
...
- **其他场景**：再次访问 / 兜底 → 横幅直接可见（不播淡入，或瞬时淡入）

## 4. 灰烬（保留 v1 现状）

- 流失稳定后启动约 180 片灰烬飘落（2026-08-10 依用户反馈 25→60→90→180）
- 不做性能限制：后台标签页不暂停灰烬、无 reduced-motion 跳过（2026-08-10 依用户反馈移除）
