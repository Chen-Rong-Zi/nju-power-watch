...
### 为什么没有我的房间？🏠
学校的缴费系统（epay）需要用户在 app 上手动添加房间后，才能查询该房间的电费。系统也是基于这些已有的条目来获取数据的——有人添加过的房间才能查到 ✅ 如果你的房间没有出现在系统里，请先在 epay app 中添加房间，系统会在每月 1 号、8 号、15 号、22 号的自动扫描中发现它 📡

### 为什么我的房间曾经有，但是最近没有电费记录了？📭
可能原因：你(或者舍友)在 app 中删除了系统原本追踪的房间条目，需要重新添加，再等到每月 1 号、8 号、15 号、22 号的自动扫描重新发现 🔄

### 数据多久更新一次？🕐
每天 14:00 自动采集更新 ⏰ 数据采集通过 GitHub Actions 自动执行，无需人工干预。

### 为什么和 epay 上看到的余额不一样？🔢
系统每天在固定时间采集一次数据，并非实时同步 ⏳ 如果你在 epay 上充值或用电后余额发生变化，需要等到下一次采集才会更新。一般情况下，余额差异不会超过一天 📅
